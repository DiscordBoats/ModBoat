    ...
    async punish(member: Member | {id: string, guild: Guild}, punishment: Punishment, reason?: string) {
        ...
        switch (punishment.type) {
            case "ban": 
                ...
                await guild.banMember(member.id, days, reason ? encodeURIComponent(reason) : undefined);
                if (soft) {
            ...
            case "kick":
                if (!(member instanceof Member))
                    return;
                await member.kick(reason ? encodeURIComponent(reason) : undefined);
                break;
            case "mute": 
                if (!(member instanceof Member))
                        return;
                ...
                await member.addRole(muterole, reason ? encodeURIComponent(reason) : undefined);
                if (!!temp) {
            ...
            case "role":
                ...
                if (!!role && !!PermissionUtils.topRole(me) && PermissionUtils.topRole(me)!.position > role.position)
                    await member.addRole(role.id, reason ? encodeURIComponent(reason) : undefined);
                break;
            case "unmute":
                ...
                if (!!muted && !!(mem! as Member).roles.find(x => x === muted.id)) {
                    await(mem! as Member).removeRole(muted.id, reason ? encodeURIComponent(reason) : undefined);
                }
                break;
            case "unban":
            ...
            case "unrole":
                ...
                if (member instanceof Member && !!srole && !!PermissionUtils.topRole(me) && PermissionUtils.topRole(me)!.position > srole!.position)
                    await member.removeRole(srole.id, reason ? encodeURIComponent(reason) : undefined);
                break;
        }
